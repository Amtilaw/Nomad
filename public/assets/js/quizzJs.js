    ...
    for (let i = 0; i < questionsSize ; i++){
        ...
        function checkPalier(){
            isUserForwardVideo();
          if (player.api("time") > json[countQuestion]["timecode"] && quizzDone == true){
       ...
       function renderMinuteur(){
        ...
        function sendUserRespond(userRes){
            ...
            let answerId = [];
          for (let i = 0; i < Object.keys(json[countQuestion]["propositions"]).length ; i++){
        ...
        let canGoToNextQuestion = validateQuestion();
                if (canGoToNextQuestion == true){
                 countQuestion++;
            if (countQuestion < Object.keys(json).length && countQuestion > 0 && json[countQuestion -1]["timecode"] == json[countQuestion]["timecode"]){
    ...
    function renderRadioQuizz(){
          //type == 1 == qcm
          if (json[countQuestion]["type"] == 1){
...
for (let i = 0; i < Object.keys(json[countQuestion]["propositions"]).length; i++){
  ...
  document.getElementById("boardProps").innerHTML += `<input type="${typeProposition}" name="box" id="proposition${i}" >
